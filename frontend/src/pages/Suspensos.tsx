...
import { suspensoService } from '@/services/api';
import { userService } from '@/services/api';
import type { User, Suspensos } from '@/types';
import { Modal } from '@/components/Modal';
import { useSocketEvent } from '../services/websocket';

// Definição do tipo para os eventos de WebSocket
interface WebSocketEvent {
  type: 'extra' | 'fisico' | 'suspenso' | 'user';
  action: 'create' | 'update' | 'delete';
  id?: string | number;
}

export function Suspensos() {
  const navigate = useNavigate();
  const [users, setUsers] = useState<User[]>([]);
  const [suspensosList, setSuspensosList] = useState<Suspensos[]>([]);
  ...
  const [suspensoToDelete, setSuspensoToDelete] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    const savedDelivered = localStorage.getItem('deliveredSuspensos');
    if (savedDelivered) {
  ...
  useSocketEvent<WebSocketEvent>('data-changed', (data) => {
    ...
    if (data.type === 'suspenso') {
      console.log('Dados de suspensos alterados:', data);
      loadDocuments();
    }
  });
  ...
  }, [suspensosList, itemsPerPage, searchTerm]);

  function toEdit(id_suspenso: number) {
    //console.log('Editando documento:', id_extra);
    navigate(`/suspenso/edit/${id_suspenso}`);
  }

  function deleteSuspenso(id_suspenso: number) {
    // Open modal and store the id to delete
    setSuspensoToDelete(id_suspenso);
    setIsDeleteModalOpen(true);
  }

  const confirmDeleteSuspenso = async () => {
    if (suspensoToDelete) {
      try {
        await suspensoService.delete(suspensoToDelete);
  ...
  const loadDocuments = async () => {
    try {
      setLoading(true);
      console.log('iniciando carregamento de documentos suspensos...');
      const suspensosDocs = await suspensoService.getAll();
      console.log('feitos suspensos carregados:', suspensosDocs);
      const processedSuspensos = suspensosDocs.map((doc: Suspensos) => {

        const receivedDate = new Date(doc.receivedAt);
        const deliveryDate = new Date(doc.deliveryDeadLine);

        console.log(receivedDate + ' e ' + deliveryDate)
      ...
      setSuspensosList(processedSuspensos);

      if (processedSuspensos.length === 0) {
        localStorage.removeItem('deliveredSuspensos');
      }

      const countSuspensos = await suspensoService.getCount()
      localStorage.setItem('suspensosCount', countSuspensos.toString());
    } catch (err) {
      setError('Erro ao carregar documentos feitos suspensos');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleInternalDeliveryChange = async (idDocument: string, userId: number) => {
    try {
      ...
      await suspensoService.updateInternalDelivery(idDocument, userId);
    } catch (error) {
  ...
  const requestSort = (key: keyof Suspensos) => {
  ...
  const filteredDocuments = suspensosList
    .filter((doc) => {
  ...
  const paginate = (pageNumber: number) => {
    ...
    const tableElement = document.getElementById('suspensos-table');
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h1 className="text-xl font-semibold text-gray-900">Feitos suspensos</h1>
          <p className="mt-2 text-sm text-gray-700">
            Lista de todos os feitos suspensos cadastrados no sistema.
          </p>
        </div>

        <Link to="/suspenso/add">
        ...
        <div className="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
              <table id="suspensos-table" className="min-w-full divide-y divide-gray-300">
                <thead className="bg-gray-50">
                  <tr>
                    <th
                      scope="col"
                      className="py-2 pl-2 pr-2 text-left text-sm font-semibold text-gray-900 sm:pl-6 cursor-pointer"
                      onClick={() => requestSort('receivedAt' as keyof Suspensos)}
                    >
                    ...
                    <th
                      ...
                      onClick={() => requestSort('idDocument' as keyof Suspensos)}
                    >
                      <div className="flex items-center group">
                        Número ID
                        <span className="ml-2 invisible group-hover:visible">
                    ...
                    <th
                      scope="col"
                      className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 cursor-pointer"
                      onClick={() => requestSort('deliveryDeadLine' as keyof Suspensos)}
                    >
                      <div className="flex items-center group">
                        Prazo de entrega
                        <span className="ml-2 invisible group-hover:visible">
                        ...
                        >
                          ...
                          <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <div className="flex justify-end space-x-2">
                              ...
                              <button
                                onClick={() => deleteSuspenso(doc.id_suspenso)}
      ...
      {isDeleteModalOpen && (
        <Modal
          ...
          onConfirm={confirmDeleteSuspenso}
